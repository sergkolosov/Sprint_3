...
class TestConstructorPages:
    ...
    def test_go_to_sauces_partition(self, driver):
        ...
        actually_value = self.flag_selected_tab in driver.find_element(*Locators.SAUCES_BUTTON_SELECT).get_attribute('class')
        expected_value = True
        assert actually_value == expected_value, f'Ожидалось, что содержание "{self.flag_selected_tab}" в выделенном табе: "{expected_value}", получено "{actually_value}"'
    ...
    def test_go_to_fillings_partition(self, driver):
        ...
        actually_value = self.flag_selected_tab in driver.find_element(*Locators.FILLINGS_BUTTON_SELECT).get_attribute('class')
        expected_value = True
        assert actually_value == expected_value, f'Ожидалось, что содержание "{self.flag_selected_tab}" в выделенном табе: "{expected_value}", получено "{actually_value}"'
    ...
    def test_go_to_buns_partition(self, driver):
        ...
        actually_value = self.flag_selected_tab in driver.find_element(*Locators.BUNS_BUTTON_SELECT).get_attribute('class')
        expected_value = True
        assert actually_value == expected_value, f'Ожидалось, что содержание "{self.flag_selected_tab}" в выделенном табе: "{expected_value}", получено "{actually_value}"'
